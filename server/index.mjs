/**
 * Tmuxpert API server — optional player accounts (Google/GitHub OAuth) and
...
export function loadConfig(env = process.env) {
  ...
  if (!sessionSecret) {
    ...
    console.warn(
      '[tmuxpert-server] SESSION_SECRET is not set — using a random one-off secret. ' +
  ...
  return {
    ...
    dbPath: env.DB_PATH || './tmuxpert.db',
    publicOrigin,
...
export function createApp(config) {
  ...
  async function authCallback(req, res, provider, url) {
    ...
    try {
      ...
      console.error(`[tmuxpert-server] ${provider} oauth callback failed:`, err?.message ?? err)
  ...
  const server = createServer((req, res) => {
    handle(req, res).catch((err) => {
      console.error('[tmuxpert-server] unhandled error:', err)
...
if (isMain) {
  ...
  app.server.listen(config.port, () => {
    console.log(
      `[tmuxpert-server] listening on :${config.port}` +
