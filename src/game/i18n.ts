...
// Kept distinct from the game save key ('tmuxlegends-save') on purpose.
const STORAGE_KEY = 'tmuxlegends-lang'

// Forward-migrate the language preference from the pre-rebrand key ('tmuxpert-lang')
...
try {
  if (localStorage.getItem(STORAGE_KEY) == null) {
    const legacy = localStorage.getItem('tmuxpert-lang')
    if (legacy != null) localStorage.setItem(STORAGE_KEY, legacy)
  }
  localStorage.removeItem('tmuxpert-lang')
} catch {
