...
export const tier4: Challenge[] = [
  {
    ...
    tier: 4,
    title: 'Cycle Layouts',
    brief: 'Your panes are stacked. Snap them into a tidy side-by-side layout with `Ctrl-b` then `Space`.',
    taughtCommands: ['next-layout'],
    start: stackedThree,
    goal: { predicate: splitDirIs('h'), describe: 'Panes tiled side by side' },
    par: 2,
    hint: '`Ctrl-b` then `Space` cycles through tmux’s preset layouts - press it to rebalance the panes.',
  },
  {
    id: 't4-swap',
    tier: 4,
    title: 'Swap Panes',
    brief: 'Your editor opened on the right. Swap it to the left with `Ctrl-b` then `}`.',
    taughtCommands: ['swap-pane'],
    start: logsLeftVimRight,
    goal: { predicate: firstPaneCmd('vim'), describe: 'The vim pane is now on the left' },
    par: 2,
    hint: '`Ctrl-b` `}` swaps the active pane with the next one; `Ctrl-b` `{` swaps with the previous.',
  },
  {
    id: 't4-break',
    tier: 4,
    title: 'Break It Out',
    brief: 'This db pane deserves its own window. Break the active pane out with `Ctrl-b` then `!`.',
    taughtCommands: ['break-pane'],
    start: threeToBreak,
    goal: { predicate: windowCount(2), describe: 'The pane became its own window' },
    par: 2,
    hint: '`Ctrl-b` `!` (break-pane) moves the active pane into a brand-new window and jumps you to it.',
  },
  {
    id: 't4-swap-cycle',
    tier: 4,
    ...
    par: 4,
    hint: '`Ctrl-b` `}` moves vim to the front, then `Ctrl-b` `Space` cycles the side-by-side split into a stacked one.',
  },
  {
    id: 't4-capstone',
    tier: 4,
    title: 'Workspace Surgery',
    brief: 'First swap vim to the left with `Ctrl-b` `}`.',
    ...
    stages: [
      {
        brief: 'Now give the server its own window: select it (`Ctrl-b` `o`), then break it out (`Ctrl-b` `!`).',
        goal: { predicate: windowCount(2), describe: 'server broken out into its own window' },
      },
    ],
    par: 6,
    hint: '`Ctrl-b` `}` swaps vim left. `Ctrl-b` `o` hops to the server pane, then `Ctrl-b` `!` breaks it into a new window.',
  },
]
