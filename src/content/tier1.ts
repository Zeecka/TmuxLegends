...
export const tier1: Challenge[] = [
  {
    ...
    tier: 1,
    title: 'The Prefix',
    brief: 'Every tmux command starts with the prefix, `Ctrl-b`. Press it, then `%` to split this pane into left and right.',
  ...
  },
  {
    ...
    tier: 1,
    title: "Stack 'em",
    brief: 'Now split top and bottom instead. `Ctrl-b`, then `"` (Shift+quote).',
    taughtCommands: ['split-v'],
    start: single({ cmd: 'zsh' }),
    goal: { predicate: allOf(paneCount(2), splitDirIs('v')), describe: 'Two panes, stacked top and bottom' },
    par: 2,
    hint: 'The mnemonic: `%` is a tall divider (left|right), `"` looks like it stacks. `Ctrl-b` then `"`.',
  },
  {
    id: 't1-navigate',
    tier: 1,
    title: 'Pane Hop',
    brief: 'Your editor is in the LEFT pane, but focus is on the right. Hop to the editor with `Ctrl-b` then `o`.',
    taughtCommands: ['select-pane'],
    start: vimThenSplitRight,
    goal: { predicate: activePaneCmd('vim'), describe: 'The vim pane is active' },
    par: 2,
    hint: '`Ctrl-b` then `o` cycles panes. (`Ctrl-b` then an arrow key moves by direction - try `←` too.)',
  },
  {
    id: 't1-zoom',
    tier: 1,
    title: 'Zoom In',
    brief: 'Too cramped? Temporarily blow the current pane up to fullscreen with `Ctrl-b` then `z`.',
    taughtCommands: ['zoom'],
    start: vimThenSplitRight,
    goal: { predicate: paneZoomed(), describe: 'The active pane is zoomed' },
    par: 2,
    hint: '`Ctrl-b` then `z` toggles zoom. Press it again later to pop back to the split - your panes are still there.',
  },
  {
    id: 't1-kill',
    tier: 1,
    title: 'Trim the Clutter',
    brief: 'Three panes is one too many. Close the current pane with `Ctrl-b` then `x`.',
    taughtCommands: ['kill-pane'],
    start: threePanes,
    goal: { predicate: paneCount(2), describe: 'Only two panes remain' },
    par: 2,
    hint: '`Ctrl-b` then `x` kills the active pane. (Real tmux asks y/n first; here it just closes.)',
  },
  {
    id: 't1-capstone',
    tier: 1,
    ...
    par: 4,
    hint: '`Ctrl-b` `%` gives you left|right and lands you in the new RIGHT pane. `Ctrl-b` `"` then splits that one top/bottom.',
  },
]
