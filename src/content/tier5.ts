...
export const tier5: Challenge[] = [
  {
    ...
    brief: 'Open tmux’s command prompt with `Ctrl-b` then `:` and run:  `splitw -v`',
    ...
    par: 12,
    hint: '`splitw` is short for `split-window`; `-v` stacks the panes. `Ctrl-b` `:` opens the prompt, then type it and `Enter`.',
  },
  {
    id: 't5-cmd-newwin',
    tier: 5,
    title: 'Named Window',
    brief: 'From the command line, make a named window. `Ctrl-b` `:` then:  `neww -n deploy`',
  ...
  },
  {
    ...
    brief: 'Select a window by number from the prompt. `Ctrl-b` `:` then:  `selectw -t 2`',
  ...
  },
  {
    ...
    brief: "The editor is window 1; move it to slot 0. `Ctrl-b` `:` then:  `swapw -t 0`",
  ...
  },
  {
    ...
    brief: 'Create a second, detached session from the prompt. `Ctrl-b` `:` then:  `new -s staging`',
  ...
  },
  {
    ...
    brief: 'Script a workspace in two commands: make an api window, then split it. `Ctrl-b` `:` then  `neww -n api` , then `Ctrl-b` `:` then  `splitw -h`',
