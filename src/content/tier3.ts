...
export const tier3: Challenge[] = [
  {
    ...
    tier: 3,
    title: 'Detach',
    brief: 'Leave everything running and step away: detach from the session with `Ctrl-b` then `d`.',
    ...
    par: 2,
    hint: "`Ctrl-b` then `d` detaches. Nothing is lost - later `tmux attach` drops you right back in.",
  },
  {
    id: 't3-rename-session',
    tier: 3,
    title: 'Name the Session',
    brief: "This session is 'main'. Give it a purpose-driven name `work`: `Ctrl-b` then `$` then type it and `Enter`.",
    ...
    par: 7,
    hint: '`Ctrl-b` then `$` opens the session rename prompt. Type `work` and `Enter`.',
  },
  {
    id: 't3-new-session',
    tier: 3,
    title: 'Second Session',
    brief: "Start a separate session for the API. Open the command prompt with `Ctrl-b` then `:` and run:  `new -s api`",
    ...
    par: 13,
    hint: "`Ctrl-b` then `:` gives you tmux's command line. `new` is short for new-session; -s sets the name.",
  },
  {
    id: 't3-copy-mode',
    tier: 3,
    title: 'Enter Copy Mode',
    brief: 'To scroll back and read old output you enter copy mode: `Ctrl-b` then `[`.',
    taughtCommands: ['copy-mode'],
    start: single({ cmd: 'logs', content: LOG_LINES }),
    goal: { predicate: inMode('copy'), describe: 'You are in copy mode' },
    par: 2,
    hint: '`Ctrl-b` then `[` enters copy mode. Now `hjkl` / arrows scroll; `q` leaves. This is how you read past output.',
  },
  {
    id: 't3-copy-yank',
    tier: 3,
    ...
    par: 5,
    hint: '`Ctrl-b` `[` to enter copy mode. `Space` starts the selection, `$` extends it to the end of the line, `Enter` (or `y`) copies.',
  },
  {
    id: 't3-capstone',
    tier: 3,
    ...
    par: 12,
    hint: '`Ctrl-b` `[` then `/` opens search. Type `ERROR` and `Enter` to jump to it, then `Space`, `$`, `y` to copy that line.',
  },
]
