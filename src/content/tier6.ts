...
export const tier6: Challenge[] = [
  {
    ...
    tier: 6,
    title: 'Give It Room',
    brief: "Your editor pane is cramped. There's no default key for it - resize from the prompt: `Ctrl-b` `:` then  `resizep -R`  (`Enter`).",
    ...
    par: 13,
    hint: '`Ctrl-b` `:` opens the command prompt. Type `resizep -R` (an alias for resize-pane) and `Enter` to grow the pane rightward. Real tmux also binds `C-b` + `Ctrl+arrow`.',
  },
  {
    id: 't6-paste',
    tier: 6,
    title: 'Paste It Back',
    brief: "You copied `npm run deploy` earlier. Drop it into this shell: paste the buffer with `Ctrl-b` `]`.",
    ...
    par: 2,
    hint: '`Ctrl-b` then `]` pastes whatever is in your copy buffer. (You fill the buffer in copy mode with `Space` ... `y`.)',
  },
  {
    id: 't6-reorder',
    tier: 6,
    title: 'Reorder Windows',
    brief: "You're on 'server' (window 2) but it belongs first. There's no key for this - reorder from the prompt: `Ctrl-b` `:` then  `swapw -t 0`  (`Enter`).",
    ...
    par: 13,
    hint: '`Ctrl-b` `:` then `swapw -t 0` swaps the current window with window 0. `swapw` is the alias for swap-window.',
  },
  {
    id: 't6-cmd-split',
    tier: 6,
    title: 'Split by Script',
    brief: 'Configs and scripts build layouts from the command line, not keybindings. Split this window left/right: `Ctrl-b` `:` then  `splitw -h`  (`Enter`).',
    taughtCommands: ['split-h', 'command-prompt'],
    start: single({ cmd: 'zsh' }),
    goal: { predicate: paneCount(2), describe: 'The window has two panes' },
    par: 12,
    hint: '`Ctrl-b` `:` then `splitw -h` (alias for split-window -h). Use -v for a top/bottom split. This is exactly what a .tmux.conf layout does.',
  },
  {
    id: 't6-capstone',
    tier: 6,
    ...
    par: 25,
    hint: '`Ctrl-b` `:` `splitw -h` `Enter` makes the pane. Then `Ctrl-b` `:` `resizep -L` `Enter` nudges the divider. Two prompt commands, one tidy layout.',
  },
]
